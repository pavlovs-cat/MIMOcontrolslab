...
typedef RT_MODEL_MLC_Aero_Simulink201_T RT_MODEL;

/*====================*
 * External functions *
 *====================*/
EXTERN RT_MODEL * MLC_Aero_Simulink2016a(void);
  ...
  _do_assertion(const char * expression, const char * file_name, int line_number)
{
  ...
  rtmSetErrorStatus(MLC_Aero_Simulink2016a_M, GBLbuf.message);
}
  ...
  control_c_handler(int signal_number)
{
  ...
  GBLbuf.stopExecutionFlag = 1;
  rtExtModeStart();
  hil_task_stop(MLC_Aero_Simulink2016a_DW.HILReadTimebase_Task)
    ;
}
  ...
  main(int argc, char * argv[])
{
  ...
  rtExtModeQuarcParseArgs(argc, (const char **) argv,
    "shmem://MLC_Aero_Simulink2016a:1");
  ...
  if (parseError) {
    (void) fprintf(stderr,
                   "\nUsage: MLC_Aero_Simulink2016a -option1 val1 -option2 val2 -option3 ...\n\n");
    (void) fprintf(stderr,
  ...
  if (rtExtModeQuarcStartup(rtmGetRTWExtModeInfo(S),
    ...
    if (rtmGetErrorStatus(S) == NULL) {
      ...
      fflush(stdout);
      result = hil_task_start(MLC_Aero_Simulink2016a_DW.HILReadTimebase_Task,
        (t_clock) MLC_Aero_Simulink2016a_P.HILReadTimebase_clock, 500.0, -1)
        ;
      if (result == 0) {
